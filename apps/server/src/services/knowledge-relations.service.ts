...
const explicitPrerequisites: Array<[string, string, string]> = [
  ...
  ['ENG-01', 'ENG-02', '开发与生产环境差异需要先理解模块图、构建产物和运行时加载'],
  ['ENG-03', 'ENG-05', '可复现质量门禁建立在锁定依赖、workspace 边界和统一脚本之上'],
  ['TS-01', 'TS-02', '基本类型和收窄是对象类型建模的前置'],
  ['TS-02', 'TS-03', '泛型需要先理解结构类型与联合收窄'],
  ['TS-03', 'TS-04', '映射类型、键重映射与模板字面量需要先掌握泛型、keyof 和索引访问'],
  ['TS-03', 'TS-05', '条件类型和 infer 建立在泛型代入、约束与联合关系之上'],
  ['TS-03', 'TS-06', '类型安全的函数与组件 API 需要先掌握泛型关系和键值约束'],
  ['TS-02', 'TS-07', '运行时契约的成功、失败和兼容状态需要联合收窄与穷尽处理'],
  ['TS-07', 'TS-08', '业务状态与权限命令进入可信核心前需要运行时契约和稳定错误模型'],
  ['TS-01', 'TS-09', '版本迁移治理需要先理解 TypeScript 的编译时边界与严格类型检查'],
  ['ENG-03', 'TS-09', '编译器迁移依赖可解释的版本解析、锁文件、workspace 和 peer 兼容边界'],
  ['JS-03', 'REACT-01', '不可变更新与值语义是理解 React 渲染的基础'],
  ['REACT-01', 'REACT-02', '组件与渲染模型是状态更新的前置'],
  ['REACT-02', 'REACT-10', '数据路由与导航状态建立在组件状态、渲染和事件模型之上'],
  ['TEST-01', 'TEST-02', '组件行为测试需要先掌握测试预言、等价类、替身与隔离原则'],
  ['REACT-02', 'TEST-02', 'React 组件测试需要先理解组件边界、数据流和可观察行为合同'],
  ['TEST-01', 'TEST-03', '端到端和视觉测试需要先掌握预言、隔离与不确定性治理'],
  ['WEB-01', 'TEST-03', '用户关键路径与可访问查询建立在语义 HTML、表单和焦点合同之上'],
  ...
  ['WEB-01', 'WEB-02', 'CSS 布局与响应式设计需要先有语义稳定的文档与控件结构'],
  ['WEB-02', 'WEB-03', '先掌握布局与层叠，再建立现代 CSS 架构'],
  ['WEB-01', 'BROWSER-01', 'DOM 渲染、事件与存储案例需要先理解语义文档和原生控件结构'],
  ['JS-04', 'BROWSER-01', '浏览器事件与 IndexedDB 异步事务依赖事件循环、任务与 Promise 基础'],
  ['BROWSER-01', 'WEB-04', '原生分层 UI 与导航增强依赖浏览器事件、渲染和历史模型'],
  ['A11Y-01', 'WEB-04', '原生浮层与视图切换必须建立在键盘、焦点和用户偏好合同之上'],
  ['BROWSER-01', 'BROWSER-02', '页面生命周期、观察器和多标签协同需要先理解 DOM、事件与浏览器存储'],
  ['BROWSER-01', 'WEB-05', 'Custom Elements 与 Shadow DOM 依赖浏览器 DOM、事件和生命周期'],
  ['BROWSER-01', 'NET-01', '浏览器请求可靠性需要先理解事件循环、缓存来源、页面身份和存储边界'],
  ['NET-01', 'SEC-01', 'Web 攻击与防护依赖源、Cookie、请求和缓存语义'],
  ['DEBUG-01', 'CAREER-04', '事故响应和复盘需要先掌握事实假设分离、因果定位与验证闭环'],
  ['GIT-03', 'CAREER-05', '代码评审与风险沟通需要先理解提交边界、PR 生命周期和合入证据'],
  ...
  ['SEC-01', 'IDENTITY-01', '会话与浏览器身份边界建立在 Cookie 自动携带、XSS、CSRF 和服务端信任边界之上'],
  ['BROWSER-02', 'PERF-03', '长任务、帧归因与现场观察依赖浏览器调度和观察器生命周期'],
  ['DEBUG-01', 'OBS-01', '生产可观测性应延续系统化假设、关联和复现方法'],
  ['NET-01', 'UMI-03', '请求层设计需要 HTTP、缓存与取消基础'],
  ['BIZ-01', 'BIZ-03', '角色和属性授权需要先明确业务对象、关系与统一语言'],
  ['BIZ-01', 'BIZ-04', '接口契约需要稳定的业务对象与操作语言'],
  ['TS-07', 'BIZ-04', '前端模型防腐需要静态类型与运行时输入校验共同守住边界'],
  ['BIZ-02', 'BIZ-05', '多视图一致性需要先区分业务状态、事件与不变量'],
  ['BIZ-04', 'BIZ-05', '表单、列表与详情必须建立在统一 DTO 转换和 API 语义上'],
  ['BIZ-02', 'BIZ-06', '长任务的排队、运行、取消和终态需要明确状态机'],
  ['JS-05', 'BIZ-06', '异步任务客户端需要掌握错误传播、取消与恢复边界'],
  ['BIZ-02', 'BIZ-07', '错误恢复、补偿与最终一致需要业务状态和非法转移边界'],
  ['BIZ-04', 'BIZ-07', '未知结果和条件写入依赖明确接口与错误契约'],
  ['BIZ-01', 'BIZ-08', '需求追踪必须先有稳定业务语言和对象边界'],
  ['ENG-05', 'BIZ-08', '验收证据追踪需要可执行质量门禁承接'],
  ...
  ['TEST-01', 'TEST-04', '契约测试同样需要测试预言、边界与替身隔离基础'],
  ['TEST-04', 'ENG-06', '跨服务发布需要 API 契约兼容的自动化证据'],
  ['ENG-06', 'OBS-01', '发布闭环必须通过真实用户监控验证生产结果'],
  ['LINUX-02', 'DEPLOY-01', 'Nginx、代理、TLS 与 CDN 排障需要进程、端口、日志和网络诊断基础'],
  ['OBS-01', 'PERF-01', 'Core Web Vitals 字段分布和性能预算需要稳定的 RUM、版本与分位数口径'],
  ['NET-01', 'PERF-02', '资源加载、缓存和导航优化需要先理解 HTTP、连接、取消与重试'],
  ['PERF-01', 'PERF-02', '网络优化应由用户指标和性能预算驱动'],
  ['PERF-01', 'PERF-03', '主线程与帧优化需要先掌握 INP 的输入、处理和呈现阶段'],
  ['JS-03', 'PERF-04', '内存保留图和垃圾回收需要先理解对象身份、引用与可达性'],
  ['BROWSER-02', 'PERF-04', '监听器、观察器与页面生命周期决定浏览器资源何时释放'],
  ['WEB-02', 'H5-01', '移动视口适配建立在内容驱动布局和响应式约束之上'],
  ['A11Y-01', 'H5-01', '缩放、重排、操作目标与焦点可见性是移动适配底线'],
  ['H5-01', 'H5-02', '软键盘与手势浮层需要先理解布局视口、视觉视口和安全区'],
  ['BROWSER-01', 'H5-02', 'Pointer 事件、默认行为和焦点协作依赖浏览器事件模型'],
  ...
  ['AIDEV-01', 'AIDEV-02', '上下文选择与仓库指令治理建立在受控任务合同、工具边界和检查点之上'],
  ['A11Y-01', 'COMP-01', '公共组件职责与 API 必须建立在语义、键盘和焦点合同上'],
  ['COMP-01', 'COMP-02', '状态所有权和命令句柄建立在清晰组件职责与公共契约上'],
  ['NET-01', 'LINUX-02', '端口、DNS、HTTP 与缓存排障依赖网络基础'],
  ['LINUX-01', 'LINUX-02', '进程、socket 和日志排障需要先掌握路径、权限与文件对象'],
  ['LINUX-01', 'LINUX-03', '安全 Shell 自动化需要先掌握路径、权限、链接和批处理边界'],
  ['SEC-01', 'LINUX-04', '服务器安全是前端安全边界的生产延伸'],
  ['LINUX-01', 'LINUX-04', '服务器账号、密钥和最小权限基线建立在文件权限基础上'],
  ['LINUX-01', 'DOCKER-01', '镜像文件、运行用户和信号边界需要 Linux 文件与权限基础'],
  ['DOCKER-01', 'DOCKER-02', '多容器编排需要先掌握镜像、构建上下文与容器生命周期'],
  ['LINUX-02', 'DOCKER-02', 'Compose 服务发现、健康和故障恢复需要进程网络诊断基础'],
  ...
  ['JS-04', 'NODE-01', 'Node 事件循环建立在任务、微任务、Promise 与宿主异步调度基础上'],
  ['NODE-01', 'NODE-02', '流和文件管线需要先理解 Node 主线程、工作池、资源与非阻塞 I/O 的分工'],
  ['JS-05', 'NODE-02', 'pipeline 的错误、取消与清理建立在异步错误传播和 AbortSignal 基础上'],
  ...
  ['PRIVACY-01', 'PRIVACY-02', '跨区域控制需要先掌握目的、最小化、留存、权利请求和下游处理方生命周期'],
  ...
  ['BIZ-07', 'DATA-02', '乐观与离线写入需要幂等、未知结果、冲突和一致性基础'],
  ['NET-01', 'REALTIME-01', '实时协议需要 HTTP 与连接生命周期基础'],
  ['BIZ-07', 'REALTIME-01', '实时消息恢复需要幂等、重复、未知结果与最终一致语义'],
  ...
  ['AIAPP-04', 'AIAPP-05', '生成式 UI 必须先把工具提议、执行事实和结果状态建模清楚'],
  ['SEC-04', 'AIAPP-05', '生成式 UI 的宿主、桥接和嵌入能力建立在跨源隔离边界之上'],
  ['AIAPP-05', 'AIUI-01', '协议互操作需要先掌握通用视图模型、组件注册表和宿主边界'],
  ['AIAPP-02', 'AIUI-01', 'Agent UI 事件与恢复需要先理解流式序号、终态和增量渲染'],
  ['AIAPP-01', 'AIAPP-06', 'RAG 生成与引用建立在模型输入输出和上下文可信边界之上'],
  ['AIPROD-01', 'AIAPP-08', 'AI 评测应从已定义的任务价值、失败分布和验收证据出发'],
  ['OBS-01', 'AIAPP-08', 'AI 语义评测和发布门禁需要可靠的版本、Trace 与生产观测基础'],
  ['AIAPP-01', 'AIAPP-09', '成本、配额和缓存需要先理解模型调用、Token 与供应商差异'],
  ['NET-01', 'AIAPP-09', '限流、重试、超时、缓存与熔断建立在 HTTP 可靠性语义之上'],
  ['AIAPP-02', 'AIAPP-10', '可信恢复体验需要先理解流式中间态、取消和断线收敛'],
  ['AIAPP-04', 'AGENT-01', 'Agent Loop 通过工具调用把推理转成行动'],
  ['MCP-01', 'AGENT-03', 'MCP 传输演进建立在 Host、Client、Server 与核心能力语义之上'],
  ['AGENT-03', 'AGENT-04', 'Client 发现与兼容必须先理解当前传输、无状态核心和版本迁移'],
  ['MCP-01', 'AGENT-08', 'Agent 工具可发现性建立在 MCP Tool、Resource 与 Prompt 契约上'],
  ['AGENT-01', 'AGENT-09', 'Agent 可观测性需要先理解一次运行中的步骤、工具动作、重试和终止状态'],
  ['OBS-01', 'AGENT-09', 'Agent 追踪、指标、日志和隐私治理建立在通用可观测性方法之上'],
  ...
  ['AIDEV-01', 'AIDEV-03', 'AI 代码验证需要先把规格、执行边界、检查点和验收证据纳入受控闭环'],
  ['TEST-03', 'AIDEV-03', 'AI 生成变更需要关键路径 E2E 兜底'],
  ['AIDEV-03', 'AIDEV-04', 'AI Code Review 需要先掌握独立验证预言、失败注入和证据闭环'],
  ['CAREER-05', 'AIDEV-04', 'AI 评审仍建立在普通代码评审的风险沟通、责任和合入边界上'],
  ['AIDEV-03', 'AIDEV-07', 'AI 生成代码与建议依赖进入仓库前需要独立验证'],
  ['ENG-08', 'AIDEV-07', 'AI 建议依赖的来源、锁文件和制品准入建立在供应链基础之上'],
  ...
  ['BIZ-07', 'MCP-01', '有副作用工具需要掌握幂等、未知结果和业务错误边界'],
  ['BIZ-08', 'AIPROD-01', 'AI 任务价值验证需要从需求追到可执行验收证据'],
  ['AIPROD-01', 'AIPROD-02', '先证明任务价值和失败分布，再决定自动化与人工控制等级'],
  ['UX-01', 'AIPROD-02', '高风险 AI 的确认、等待、拒绝和恢复依赖完整交互状态'],
  ['BIZ-07', 'AIPROD-02', '审批执行需要处理幂等、未知结果、补偿和最终一致'],
  ['AIPROD-01', 'AIAPP-01', '先验证 AI 任务价值，再进入模型应用实现'],
  ['SEC-01', 'AISAFE-01', 'AI 输出安全建立在通用不可信输入输出边界上'],
  ['AISAFE-01', 'AISAFE-02', '掌握防护链后再进行系统化红队'],
  ['AIPROD-02', 'AIGOV-01', 'AI 治理强度需要先按后果、可逆性和人工控制等级分级'],
  ['PRIVACY-02', 'AIGOV-01', '数据来源、地区规则和权利路径需要跨地区工程控制基础'],
  ['NET-01', 'AIAPP-02', '流式 HTTP、取消、重试和断线恢复需要浏览器网络可靠性基础'],
  ...
  ['BIZ-01', 'BIZ-02', '状态机中的对象身份、不变量与业务语言建立在领域模型基础上'],
  ['TS-02', 'BIZ-02', '用判别联合和穷尽检查表达有限业务状态需要联合类型与收窄基础'],
  ['A11Y-01', 'UX-01', '可用性设计必须以可访问的语义、键盘和反馈为底线'],
  ['BIZ-02', 'UX-01', '完整交互状态和恢复路径需要业务状态机与不变量约束'],
  ...
  ['TEST-03', 'COMPAT-01', '跨浏览器自动化、视觉差异和真机矩阵建立在端到端测试隔离与稳定性基础上'],
  ['WEB-03', 'COMPAT-01', 'Baseline 与渐进增强需要先理解现代 CSS 能力、回退和内容驱动布局'],
  ...
  ['BIZ-08', 'ARCH-01', '质量属性与架构权衡需要从业务需求追到可执行验收证据'],
  ['OBS-01', 'ARCH-01', '架构质量场景需要可解释的指标、分位数、版本和生产反馈'],
  ['ARCH-01', 'ARCH-02', '架构评审需要明确质量属性与约束'],
  ['ARCH-02', 'ARCH-03', '渐进迁移需要可追溯决策和退出条件'],
  ['ARCH-01', 'ARCH-04', '技术债的影响、利息和排序需要映射到可测质量属性与业务约束'],
  ['ARCH-03', 'ARCH-05', '跨框架迁移需要先掌握垂直切片、兼容层、遥测和回滚'],
  ['ARCH-04', 'ARCH-05', '框架生命周期决策需要依赖支持矩阵、弃用窗口和债务治理'],
  ['ARCH-02', 'LEAD-01', '技术路线和跨团队推动需要可追溯决策、异议、责任与复查机制'],
