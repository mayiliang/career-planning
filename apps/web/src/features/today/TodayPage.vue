...
import { apiClient, type LearningWorkspace, type KnowledgePointListItem } from '@/api/client';
...
<template>
  <div class="learning-workspace">
    ...
    <template v-else-if="workspace">
      ...
      <section v-else class="empty-current">
        <div><small>NO CURRENT TOPIC</small><h2>现在没有正在学习的知识点</h2><p>不需要补课，也没有逾期。{{ workspace.suggested ? `统一路线建议从 ${workspace.suggested.code} · ${workspace.suggested.title} 开始。` : '想学时再从完整知识体系挑一个。' }}</p></div><div class="empty-actions"><button v-if="workspace.suggested" @click="startPoint(workspace.suggested.code)">从建议起点开始</button><button class="primary" @click="router.push('/knowledge/map')">自由选择</button></div>
      </section>

      <section class="principle-strip">
        <strong>这里没有每日任务表</strong><span>{{ workspace.principle }}</span><button @click="router.push('/plan')">查看可选 64 周路线参考</button>
      </section>
  ...
  <div v-if="showCheckin" class="dialog-backdrop" @click.self="showCheckin = false">
    <form class="checkin-dialog" @submit.prevent="saveCheckin">
      <header><div><small>CHECK-IN · {{ today }}</small><h2>今天实际学了什么？</h2><p>可以选择多个知识点。系统只记录事实，不检查计划。</p></div><button type="button" @click="showCheckin = false">×</button></header>
      ...
      <footer><span>已选 {{ checkinForm.pointCodes.length }} 个知识点</span><button type="button" @click="showCheckin = false">取消</button><button class="primary" :disabled="checkinSaving" type="submit">{{ checkinSaving ? '保存中…' : '保存今日打卡' }}</button></footer>
    </form>
  </div>
</template>
...
.empty-actions{display:flex;gap:8px}@media(max-width:600px){.empty-actions{width:100%;flex-direction:column}}
