...
- 每个知识点包含至少两份可交叉验证的资料且至少一份中文主资料，并具备覆盖范围、五类首考题、通过标准和显式耗时。
...
内容门禁会验证领域和知识点数量、编号唯一性、资料数量、中文主资料覆盖、中文讲义章节、考核题源、耗时合同、陈旧链接黑名单和 Windows CRLF 兼容。服务端测试会验证导入、硬前置方向、48 周排期、每日容量和延迟复测。
