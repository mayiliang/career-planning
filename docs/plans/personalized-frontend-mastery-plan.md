...
这次重排以最新 15 个领域、143 个知识点、知识图谱前置关系和分阶段耗时为依据。完整路线预计需要 **666 小时首次掌握 + 93 小时延迟复测**；前 15 周覆盖全部知识点，第 16 周不再塞新知识，而是用毕业项目验证跨领域迁移能力。计划把每周新知识控制在 28.5–48 小时，并把资料、练习、项目和首考切成不超过 480 分钟的每日负载块。
...
| 7 | 工程与平台 | `TEST-01` 至 `PERF-04` | get_apidoc 质量与性能流水线 | 测试、发布、RUM 与性能诊断闭环 |
| 8 | 工程与平台 | `H5-01` 至 `COMP-03` | 移动端组件与设计 Token 专项 | H5/Hybrid 修复和组件 API 证据 |
| 9 | 工程与平台 | `PLATFORM-01` 至 `MCP-01` | get_apidoc MCP 工具链 | 平台、CLI、Schema、类型生成与首个 Tool |
| 10 | AI 原生能力 | `MCP-02` 至 `AIAPP-07` | Career Atlas DeepSeek 考核链路 | MCP 安全与 AI 应用核心链路 |
| 11 | AI 原生能力 | `AIAPP-08` 至 `AGENT-05` | 可恢复的学习评测 Agent | 可信体验、评估与 Agent Loop |
| 12 | AI 原生能力 | `AGENT-06` 至 `WEBAI-03` | 最小权限 Agent 与本地推理实验 | Agent 治理与浏览器 AI 基础验证 |
| 13 | AI 原生能力 | `WEBAI-04` 至 `AIDEV-01` | 本地语义搜索 PWA | 本地推理链路与规格驱动研发起点 |
| 14 | AI 原生能力 | `AIDEV-02` 至 `AIDEV-09` | AI 研发质量门禁 | 验证、评审、可观测性与供应链治理 |
| 15 | 影响力转译 | `AIDEV-10`、`CAREER-01` 至 `CAREER-06` | 四个代表项目资产包 | 团队治理、项目资产与面试表达 |
| 16 | 综合闸门 | 九个跨领域关键节点综合复用 | Career Atlas 2.0 | 可运行产品、测试/性能报告、架构文档和演示答辩 |

### 每周 7 天固定学习节奏

每天仍按 8 小时执行，但每天的目的不同，避免把七天都做成低质量“看资料”。
...
| 周日 | 周闸门 | 90 分钟闭卷 + 120 分钟实作 + 项目证据答辩 | 周考报告、可演示里程碑 |
...
- 每周至少保留一项可运行项目证据。第 15 周才把这些证据集中转成简历和面试表达，求职模块保持支线定位。
- 第 16 周完成 4 小时综合实作和 90 分钟答辩；7 天后盲测仍通过，才算完成整个路线。
