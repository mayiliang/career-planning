...
- 严格考核：首考题 1（资料定位）：只允许使用《中文核心讲义》，定位渐进复杂度、规模建模、测量方法、工程边界和反例；首考题 2（机制解释）：闭卷解释为何相同 Big-O 仍可能表现不同，以及一次小样本跑分为何不能证明可扩展；首考题 3（最小产出）：为线性、对数、线性对数和平方增长各构造一个前端数据处理实验，给出推导、规模曲线、内存与主线程证据；首考题 4（受限排错）：仅根据输入规模、调用次数、火焰图和内存数据定位一个复杂度退化；首考题 5（学习复述）：3 分钟说明何时优化算法、何时分块/Worker、何时转移到服务端。命题边界：不考竞赛技巧；必须同时给出推导和测量证据。
...
- 严格考核：首考题 1（资料定位）：只允许使用《中文核心讲义》《中文｜MDN Map》《中文｜MDN Set》，定位结构语义、典型操作和退化边界；首考题 2（机制解释）：闭卷比较数组、Map、Set、堆、树和图的查询、写入、遍历及内存取舍，并说明算法不变量；首考题 3（最小产出）：实现稳定排序/二分边界、动态 Top-K、依赖拓扑排序和树形权限查询，配套示例测试与属性测试；首考题 4（受限排错）：处理重复值、空输入、环、递归爆栈、错误比较器和哈希身份错误；首考题 5（学习复述）：3 分钟说明为何选择该结构而非熟悉的数组。命题边界：不要求冷门竞赛模板；考核可迁移模式、正确性和取舍。
...
- 严格考核：首考题 1（资料定位）：只允许使用《MDN 异步 JavaScript》《MDN 微任务指南》《中文补充讲义：JS-04》，分别摘出定义、关键机制、边界/反例，并标明链接；首考题 2（机制解释）：闭卷说明一个浏览器任务完成后微任务检查点、渲染机会和下一任务的关系，并区分该机制与上限/取消这一应用调度协议；首考题 3（最小产出）：完成 10 道给定浏览器 trace 的输出题（每题写出任务/微任务队列变化），并按讲义的 `nextIndex`/`active`/槽位/`AbortSignal` 协议实现可取消、并发上限为 2 的浏览器任务执行器；输入为 5 个给定延迟/成功失败任务，输出必须按输入序号汇总结果、取消后不得启动新任务；首考题 4（受限排错）：针对微任务饥饿、错误的 `await` 顺序、上限被突破或取消后仍提交状态，提交 trace、预期/实际、三项可证伪假设、根因、修复与回归；首考题 5（学习复述）：3 分钟说明浏览器调度机制与可验证证据。命题边界：不得以 Node 专有调度顺序作答。
...
- 严格考核：首考题 1（资料定位）：只允许使用《中文核心讲义》《MDN CSP 指南》《Trusted Types API》《MDN upgrade-insecure-requests》，分别定位脚本执行条件、DOM sink policy、CSP 指令边界与违规报告；首考题 2（机制解释）：闭卷解释 CSP、Trusted Types、Report-Only/强制策略与报告管道各自控制的链路，以及为何不能替代输出编码和服务端授权；首考题 3（最小产出）：为含第三方 SDK、Tag Manager、Markdown 和动态脚本的应用从 Report-Only 推进到强制 CSP，接入 Trusted Types 与违规上报，并验证 DOM XSS、未知源脚本、错误 nonce、危险 sink 和恶意嵌入被阻止；首考题 4（受限排错）：定位 nonce 缓存复用、policy 过宽或遗漏、第三方动态加载、违规报告噪声、`frame-ancestors` 或 `upgrade-insecure-requests` 指令误用；首考题 5（学习复述）：3 分钟说明第三方脚本为何等同于站点权限及其准入/退出方案。命题边界：不得用广泛 allowlist、`unsafe-inline`/`unsafe-eval` 或长期 Report-Only 规避兼容问题；SRI、Mixed Content 与 HTTPS 部署不在本点命题。
- 通过标准：策略不依赖广泛 `unsafe-inline`/`unsafe-eval`；合法路径无误报；第三方脚本有 owner、必要性、CSP 来源/nonce 和退出方案；Trusted Types policy 最小且可审计；违规报告可定位版本和页面并被脱敏去重；DOM XSS、未知源脚本、错误 nonce、恶意嵌入测试失败且可观测；能说明这些控制是纵深防御。评估边界：只设置响应头、只在本地 HTTPS 运行或让浏览器控制台无报错不能通过。
- 预计耗时：资料 120 分钟；练习 180 分钟；项目 150 分钟；考核 105 分钟；复测 90 分钟
...
- 严格考核：首考题 1（资料定位）：只允许使用《中文补充讲义：SEC-05》《中文｜MDN SubtleCrypto》，分别定位 AES-GCM 参数、`CryptoKey` 的 extractable/usages、导入导出、浏览器安全上下文、轮换和失败路径，并标明章节；首考题 2（机制解释）：闭卷区分编码、哈希、MAC、签名、加密和密码派生，并解释浏览器内密钥为何不能自动成为可信根；首考题 3（最小产出）：按讲义实现带版本的 AES-GCM 本地加密信封与受控签名验签 fixture，验证 IV/AAD 篡改、不可导出 key、轮换、撤销、恢复和服务端验证流程；首考题 4（受限排错）：仅依据算法、IV/nonce、编码、密钥用途、导入导出和来源证据定位验签失败、nonce 重用或密钥泄漏；首考题 5（学习复述）：3 分钟说明哪些密码学操作可放前端、哪些必须交给服务端或托管密钥系统。命题边界：禁止自创算法或把混淆当加密；英文原文仅用于版本核验，不作为独立首考题源。
