...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[PERF-04 中文核心讲义](../chinese-guides/perf-04-memory-listeners-resource-leaks.md#perf-04)、[Chrome Memory Problems](https://developer.chrome.com/docs/devtools/memory-problems/?hl=zh-cn)、[MDN Memory Management](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Memory_management)。覆盖分工：本地讲义=GC 可达性、retaining path、资源所有权、清理与趋势验收；官方资料=DevTools 操作和内存模型查证。覆盖范围：堆快照、分配时间线、监听器、定时器、闭包、detached DOM、Blob/媒体和缓存释放；覆盖泄漏复现、增长趋势和生命周期回收。
