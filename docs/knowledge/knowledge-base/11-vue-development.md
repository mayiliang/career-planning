...
- 严格考核：挑战类型：DESIGN_CASE；首考题 1（资料定位）：只允许使用《Props》《Component Events》《Slots》《Component v-model》，定位单向流和组合边界；首考题 2（机制解释）：闭卷为“筛选器的 query、页码、结果项插槽”指定 prop、`update:*`、业务事件和 slot prop 的所有权，说明受控父组件如何拒绝子组件的非法更新；首考题 3（最小产出）：固定夹具为 API 需求：含 `v-model:query`、`v-model:page`、可替换 result slot 与 loading/error；提交 TS Props/Emits/slot 声明、父子交互测试、6 个反例审查和最小示例；预期观察=父组件是 query/page 的唯一真相源，非法 prop 修改和不匹配载荷被类型或测试拒绝，替换 slot 仍取得完整类型；验证=运行父子交互测试与 6 个反例审查，核对事件序列、类型检查和最小示例；首考题 4（受限排错）：仅可改组件边界；注入子组件直接改 prop、emit 名称/载荷不一致、slot prop 漏类型、多模型互相覆盖。提交事件序列、三项假设、根因/修复/回归；失败现象：子组件直接改 prop，事件载荷与声明不一致；首考题 5（掌握挑战）：解释为何该场景不应放全局 store，并评审一个把“关闭弹窗”误命名成 `update:modelValue` 的 API。复测变式：仅将父组件确认 `query` 更新的延迟设为 200 ms；不变量=API 需求、`page`、slot、父组件所有权和原始 query 值不变；预期变化=子组件只发出 `update:query`，不持久化未确认的冲突值；证据=事件序列、父子交互测试和状态快照。命题边界：只在所列资料和覆盖范围内命题。
- 通过标准：数据单向、事件/载荷有 TS 契约（证据=类型/测试）；slot prop 可编译且可替换（证据=示例）；四类失败有可复现回归（证据=事件序列）；组件未借全局 store 逃避通信（证据=依赖图）。否决项：直接修改 prop、语义模糊 emit、或仅靠截图证明受控行为，均不能通过。 评估边界：仅以本点资料、题目输入与上述可复核产出评分，资料外经验不能替代证据。
- 预计耗时：资料 105 分钟；练习 165 分钟；项目 150 分钟；考核 90 分钟；复测 75 分钟
...
- 严格考核：挑战类型：DEBUGGING；首考题 1（资料定位）：只允许使用《Lifecycle Hooks》《Watchers》，定位清理、flush 和反例；首考题 2（机制解释）：闭卷解释参数 A→B→C 快切时 `watch` 的失效回调、AbortSignal 与 `flush` 如何保证只有 C 可提交；首考题 3（最小产出）：固定夹具为 A/B/C 延迟 300/200/100 ms 的请求及深层 5 000 项对象；提交可取消同步、事件/请求时间线、卸载后监听器计数和性能记录；预期观察=最终仅 C 的 100 ms 响应可提交，卸载后监听器归零且深层对象不触发无界监听；验证=重放 A/B/C 时间线和卸载步骤，核对请求日志、DOM 断言、监听器计数和性能记录；首考题 4（受限排错）：受限范围：仅可改副作用层，注入旧响应晚到、卸载后 resolve、`watchEffect` 隐式多依赖和 `deep:true` 卡顿；提交预期/实际、三项假设、根因、修复/回归；首考题 5（掌握挑战）：解释 computed、watch、watchEffect 的选择，而非把生命周期翻译成 `useEffect`。复测变式：仅把 C 请求的 100 ms 成功响应改为 100 ms 失败响应；不变量=A/B 的旧成功响应均不可覆盖当前错误状态，预期变化=页面显示 C 的可恢复错误，证据=请求时间线与 DOM 断言。命题边界：只在所列资料内。
- 通过标准：所有 I/O/监听可清理（证据=卸载计数）；快切最终只显示 C（证据=时间线）；深层监听代价可解释并有边界（证据=性能记录）；四夹具回归通过。否决项：忽略过期响应、以 `deep:true` 解决一切、或只验证一次请求，均不能通过。 评估边界：仅以本点资料、题目输入与上述可复核产出评分，资料外经验不能替代证据。
- 预计耗时：资料 105 分钟；练习 165 分钟；项目 150 分钟；考核 90 分钟；复测 75 分钟
...
- 严格考核：挑战类型：CODING；首考题 1（资料定位）：只允许使用《动态路由匹配》《Route Lazy Loading》《Navigation Guards》，定位匹配、守卫和反例；首考题 2（机制解释）：闭卷画 `/team/7/knowledge/42?tab=note` 的匹配、鉴权、懒加载、数据失败、404/重定向路径，说明客户端守卫为何不是服务端授权；首考题 3（最小产出）：固定夹具含深链、未保存草稿、无权限 403 和缺块懒加载；提交路由表、E2E 截图/日志、刷新恢复/标题/404 断言；预期观察=深链可恢复，403 与懒加载失败进入可恢复页面，草稿和查询仅在定义的路由边界内保留；验证=重放四个固定场景，核对路由表、E2E 截图/日志、刷新/标题/404 断言和服务端 403；首考题 4（受限排错）：受限范围：仅可改路由/守卫层，注入 redirect loop、重复导航、草稿丢失和过期 token；提交导航时间线、三项假设、根因/回归；首考题 5（掌握挑战）：说明何种逻辑可在守卫，何种必须在服务端。复测变式：仅将同级目标路由的授权结果从允许改为 403；不变量=详情路由、查询条件、未保存草稿、路由表和服务端授权规则不变；预期变化=导航进入可恢复的无权限状态，既有草稿和查询不越过定义边界；证据=导航时间线、E2E 截图、路由断言和 403 响应记录。命题边界：只在所列资料和覆盖范围内命题。
- 通过标准：深链、404、懒加载错误均可恢复（证据=E2E）；守卫无隐藏写副作用（证据=时间线）；草稿/查询恢复按定义工作（证据=断言）；授权最终被服务端拒绝（证据=403 fixture）。否决项：守卫循环、仅前端保护、或只验证首页跳转，均不能通过。 评估边界：仅以本点资料、题目输入与上述可复核产出评分，资料外经验不能替代证据。
- 预计耗时：资料 105 分钟；练习 165 分钟；项目 150 分钟；考核 90 分钟；复测 75 分钟
...
- 严格考核：挑战类型：CODING；首考题 1（资料定位）：只允许使用《本地中文讲义：VUE-11》《Nuxt Data Fetching》，定位 SSR/payload/key 边界；首考题 2（机制解释）：闭卷画出首屏服务端取数、payload 接管、客户端导航重取的时序，说明 key 与认证上下文为何不能混用；首考题 3（最小产出）：固定输入为 SPA 列表/详情、匿名/登录两身份和一条服务端私有 token；提交 Nuxt SSR 页面、HTML/payload 对照、导航网络记录、错误页/键盘路由播报和构建报告；预期观察=首屏 HTML、payload 与客户端导航数据一致，匿名与登录缓存按身份隔离，私有 token 不出现在 HTML、payload 或客户端请求；验证=按两种固定身份重放首屏和导航，核对 HTML/payload 对照、网络记录、产物扫描、错误页/键盘播报和构建报告；首考题 4（受限排错）：受限范围：仅可改 SSR/数据层，注入 hydration mismatch、重复请求、错误 key 串身份、私密 token 出现在 payload；提交原始响应、三项假设、修复回归；首考题 5（掌握挑战）：按内容新鲜度/个性化解释 SSR、CSR、预渲染取舍。复测变式：仅将导航后的身份从匿名改为登录；不变量=首屏匿名输入、列表/详情数据、数据 key 结构、私有 token 和 SSR 配置不变；预期变化=登录导航使用独立身份缓存且不读取匿名数据，私有 token 仍不暴露；证据=HTML/payload 对照、导航网络记录、缓存键快照和产物扫描。命题边界：只在所列资料和覆盖范围内命题。 英文原文仅用于版本核验，不作为独立首考题源。
- 通过标准：三阶段数据一致（证据=HTML/payload/网络）；私密值未出客户端（证据=产物扫描）；四夹具回归（证据=响应/日志）；路由/键盘错误路径可恢复（证据=E2E）。否决项：只看首屏、共享认证缓存、或把 hydration warning 忽略，均不能通过。 评估边界：仅以本点资料、题目输入与上述可复核产出评分，资料外经验不能替代证据。
- 预计耗时：资料 105 分钟；练习 165 分钟；项目 150 分钟；考核 90 分钟；复测 75 分钟
