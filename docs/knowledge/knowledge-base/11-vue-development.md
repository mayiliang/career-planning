...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：VUE-02 ref、reactive、computed 与边界](../chinese-guides/vue-02-ref-reactive-computed-boundaries.md#vue-02)、[Reactivity Fundamentals](https://cn.vuejs.org/guide/essentials/reactivity-fundamentals.html)、[Computed Properties](https://cn.vuejs.org/guide/essentials/computed.html)、[Reactivity in Depth](https://cn.vuejs.org/guide/extras/reactivity-in-depth.html)。覆盖范围：独立主讲义从依赖图解释 `ref`、`reactive`、`computed` 的跟踪/触发、模板解包、身份、解构丢失、深浅代理、readonly、集合、外部对象和调试钩子；头部只直接链接 VUE-01。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：VUE-03 模板、指令、事件与表单](../chinese-guides/vue-03-template-directives-events-forms.md#vue-03)、[Template Syntax](https://cn.vuejs.org/guide/essentials/template-syntax.html)、[Event Handling](https://cn.vuejs.org/guide/essentials/event-handling.html)、[Form Input Bindings](https://cn.vuejs.org/guide/essentials/forms.html)。覆盖范围：独立主讲义从模板生成 DOM 的机制串起文本/HTML 信任、属性/property、条件/列表身份、事件协议、`v-model`、编辑中间态、IME、可访问表单、调度与自定义指令边界。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：VUE-08 Pinia 与状态分层](../chinese-guides/vue-08-pinia-state-layers.md#vue-08)、[Pinia 定义 Store](https://pinia.vuejs.org/zh/core-concepts/)、[Testing Stores](https://pinia.vuejs.org/zh/cookbook/testing.html)。覆盖范围：独立主讲义先区分组件、URL、客户端领域、服务器与会话状态，再讲 store/state/getter/action、解构、持久化迁移、订阅、乐观更新、SSR 隔离、敏感数据、生命周期与测试；“全局可访问”不等于应放 Pinia。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：VUE-11 Nuxt 全栈渲染、数据与性能](../chinese-guides/vue-11-nuxt-rendering-data-performance.md#vue-11)。覆盖范围：独立主讲义画出服务端取数—payload—Hydration—客户端导航，比较 SSR/CSR/预渲染/混合路由，讲清数据 key、runtime config、Server Route、身份缓存、route rules、流式、部署适配器、可观测性和安全升级；截至 2026-08-30 的文档基线为 Nuxt 4.5.2，Nuxt 3 已于 2026-07-31 EOL，实施时必须重查官方公告、锁文件和目标环境。
