...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：TS-01 类型系统、结构化类型与严格模式](../chinese-guides/ts-01-type-system-structural-strict-mode.md#ts-01)。覆盖范围：用合法写法、预期类型错误和运行结果对照，讲清类型擦除、推断、字面量拓宽、as const/satisfies、结构兼容、品牌、额外属性、any/unknown、可选字段、索引缺失、readonly、strict 配置与断言边界，并提供完整外部输入解析。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：TS-02 联合类型、收窄、never 与穷尽检查](../chinese-guides/ts-02-unions-narrowing-never-exhaustiveness.md#ts-02)。覆盖范围：从联合的可能性和具体状态出发，讲解交叉冲突、控制流、真假/typeof/in/instanceof/相等收窄、判别联合、守卫真假两侧、断言函数、异步别名、never、新增状态的穷尽检查、开放协议解析、请求编号状态转换与精确过滤。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：TS-03 泛型、约束、`keyof` 与索引访问](../chinese-guides/ts-03-generics-constraints-keyof-indexed-access.md#ts-03)。覆盖范围：从资料列表出发，用完整类型与输出对照讲解泛型参数、最小约束、约束返回陷阱、keyof、索引访问、可选字段、typeof、推断拓宽、外部字段允许列表、异构列和闭包构造器，并解释类型参数的位置与默认值；直接前置为 TS-02。
