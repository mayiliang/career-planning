...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：TS-01 类型系统、结构化类型与严格模式](../chinese-guides/ts-01-type-system-structural-strict-mode.md#ts-01)。覆盖范围：独立主讲义从静态检查与类型擦除开始，连续讲解推断、上下文类型、拓宽、`as const`/`satisfies`、结构化可赋值性、额外属性检查、宽类型、可选/空值/索引访问、严格配置与迁移、断言风险、运行时校验和可信核心；头部只直接链接 JS-03，由它递归包含对象、函数和变量基础。
- 严格考核：挑战类型：CODING；首考题 1（资料定位）：只允许使用《中文主讲义：TS-01 类型系统、结构化类型与严格模式》，定位结构化可赋值、额外属性检查、`unknown` 与严格选项的段落，分别写出定义、机制和一个运行时反例；首考题 2（机制解释）：给定 `const literal: User={id:1, role:'admin'}` 与 `const value={id:1,role:'admin'}; const user:User=value`，解释两条语句结果不同，以及静态检查为何不能验证网络 JSON；首考题 3（最小产出）：给定固定 fixture `raw=[{id:1,name:'Ada'},{id:'2',name:'Bo'},{id:3,name:null},{id:4,name:'Di',role:'admin'}]` 和宽松 `any` 导入函数，改写为 `unknown` 入口、`User` 守卫和严格 `tsconfig`；交付迁移代码、四个输入的预期/实际记录、`tsc --noEmit` 结果和拒绝原因清单，并以测试验证合法项仅保留第一项；首考题 4（受限排错）：给定异常“字面量带 `role` 报错，但经变量赋值后进入函数并在 `name.toUpperCase()` 处崩溃”；仅在额外属性检查、`unknown` 收窄、`strictNullChecks` 三项中提出至多三项假设，逐项给出证伪证据、根因修复和四样本回归；首考题 5（学习复述）：用 3 分钟说明结构化兼容、类型擦除和运行时校验的责任边界，并回答何时不得仅凭对象形状接受数据。复测变式：仅将合法样本的 `id:number` 改为品牌化 `UserId`，保持四个输入、守卫规则和严格配置不变；预期合法项仍仅一条，提交编译记录、守卫结果与拒绝证据。命题边界：只在本点资料和给定 fixture 内命题。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：TS-02 联合类型、收窄、never 与穷尽检查](../chinese-guides/ts-02-unions-narrowing-never-exhaustiveness.md#ts-02)。覆盖范围：独立主讲义从联合与交叉的值集合模型开始，逐步讲解控制流、`typeof`/真假/相等/`in`/`instanceof` 收窄、判别联合、自定义守卫与断言函数、别名和异步边界、`never`、穷尽检查、外部开放协议、状态机及反例测试；头部只直接链接 TS-01，其 JavaScript 基础继续递归展开。
- 严格考核：挑战类型：CODING；首考题 1（资料定位）：只允许使用《中文主讲义：TS-02 联合类型、收窄、never 与穷尽检查》，定位判别联合、控制流收窄、自定义守卫和 `never` 穷尽检查的章节，并各摘出一个错误边界；首考题 2（机制解释）：给定订单结果 `loading|success|empty|error|forbidden`，解释 `kind`、守卫与 `assertNever` 如何分别防止遗漏，且说明断言为何不能替代未知输入校验；首考题 3（最小产出）：固定 fixture 为五个订单结果和额外 `{kind:'archived', archivedAt:'2026-08-01'}`；编写判别联合、`isOrderResult` 守卫、渲染 `switch` 与 `assertNever`，交付类型测试、六样本运行结果及“未处理 archived 必须编译失败”的记录；首考题 4（受限排错）：给定异常“新增 `archived` 后 `default` 静默返回空页”；仅排查判别字段是否被拓宽、是否使用断言、`assertNever` 是否可达三项，提交每项假设的证伪测试、根因和新增状态回归；首考题 5（学习复述）：用 3 分钟比较类型谓词、断言与真实 JSON 守卫，回答服务端返回未知 `kind` 时的安全顺序。复测变式：仅在固定输入中加入未知 `kind:'paused'`，保持 `forbidden` 结构、既有五种状态和守卫规则不变；预期穷尽检查仍覆盖已知状态且运行时拒绝 `paused`；提交新的编译失败记录、运行时拒绝断言和六样本输出作为新证据。命题边界：只使用给定订单状态和本点资料。
