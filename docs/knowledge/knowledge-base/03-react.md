...
- 严格考核：挑战类型：TOOL_OPERATION；首考题 1（资料定位）：只允许使用《React 19.2》《React Compiler v1.0》《React Server Components Security》，定位 Compiler 配置、手工 memo、Activity 与服务端输入安全依据；首考题 2（机制解释）：按“编译配置→组件变换→props 比较→commit 轨迹”与“client 输入→Server Function→会话再校验→允许/拒绝”两条链路说明优化和信任边界；首考题 3（最小产出）：固定 fixture 为 1,000 行列表、一个 props 依赖不完整的手工 memo、Server Function 输入 `{role:'admin', userId:'u2'}`；启用 Compiler，交付构建/lint/运行版本记录、Performance Tracks 前后比较、删除 memo 的反证、Activity 切换记录以及服务端按会话重新校验 role 的审计结论，并以筛选结果、活动状态与拒绝伪造 role 的断言验证；首考题 4（受限排错）：给定现象“Compiler 已启用但切换筛选后 props 不更新，Server Function 接受 client role”，仅排查“Compiler 未生效/版本不配”“memo 依赖遗漏”“Activity 状态保留”“服务端信任客户端输入”四个候选；提交轨迹或日志、逐项证伪、根因、修复和筛选/10 行/伪造 role 回归；首考题 5（学习复述）：3 分钟解释 Server Component 为何不是授权边界，以及何时保留手工 memo。复测变式：仅将 Server Function 的 client 输入从 `{role:'admin', userId:'u2'}` 改为 `{role:'viewer', userId:'u2'}`，保留 1,000 行列表、Compiler 与会话中的真实角色；预期服务端依真实会话而非 client role 决定允许/拒绝，提交新的请求/审计日志和授权断言。命题边界：只在「React 19.2、Compiler 与服务端边界安全」资料、固定工具记录和输入内评分；英文资料不得作为独立首考题源。
- 通过标准：配置、版本、轨迹、反证与服务端再校验记录完整；行为不回归且伪造 role 被拒绝。否决项：只看配置不测运行、将 Server Component 当授权、无前后对照、没有安全候选证伪或回滚说明。评估边界：不评估完整部署，仅评估 fixture 中的 Compiler 与信任边界。
- 预计耗时：资料 105 分钟；练习 165 分钟；项目 150 分钟；考核 90 分钟；复测 75 分钟
