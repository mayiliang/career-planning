...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：BIZ-01](../chinese-guides/biz-01-domain-objects-relations-ubiquitous-language.md#biz-01)、[Microsoft DDD 领域模型](https://learn.microsoft.com/zh-cn/dotnet/architecture/microservices/microservice-ddd-cqrs-patterns/microservice-domain-model)。覆盖范围：独立主讲义围绕业务决定，以自编课程场景解释身份、值对象、带生命周期的关系、聚合不变量、统一语言和限界上下文；可运行例子展示时间范围、报名结果及未知状态映射，继续说明应用协调、事件、并发存储、兼容迁移和指标语义。原考核材料标签与业务题例保持，正文不按题目步骤组织。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文主讲义：BIZ-02](../chinese-guides/biz-02-state-machines-business-invariants.md#biz-02)、[XState Statecharts 概念](https://stately.ai/docs/state-machines-and-statecharts)（英文原文，仅用于版本核验）。覆盖范围：独立主讲义以课程邀请的转移表、判别状态和时间守卫解释路径与不变量，并提供独立桌面观察页，观察旧版本冲突、成功重放、同键异载荷、截止边界及通知失败恢复；补充原子提交、outbox、未知结果、补偿、并行区域、迁移与可重放证据。内存模拟不冒充数据库、身份认证或真实通知服务；英文原文只用于概念与版本核验，不作为必读内容，也不得作为独立首考题源。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：BIZ-03](../chinese-guides/biz-03-rbac-abac-data-permissions.md#biz-03)。覆盖范围：独立主讲义围绕资料编辑、审核与临时委托，解释 RBAC、ABAC、数据范围、字段能力、查询前过滤、默认拒绝与实际执行；以完整策略判断、分页反例和缓存失效例子说明角色不足以证明对象权限，继续覆盖关系继承、跨租户边界、权限快照、反例与策略迁移。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：BIZ-04](../chinese-guides/biz-04-api-contract-dto-frontend-model.md#biz-04)。覆盖范围：独立主讲义从 DTO、领域与展示模型出发，以完整解析器保留缺失、null、零值和未知状态，说明 PATCH 清空语义、错误恢复、版本与单位、ETag/If-Match、Protobuf 字段存在性、Webhook、事件和兼容迁移；官方资料只作协议语义核验，业务约定用明确的自编合同说明。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：BIZ-05](../chinese-guides/biz-05-form-table-detail-state-consistency.md#biz-05)。覆盖范围：独立主讲义区分服务端事实、比较起点、当前草稿和发送快照，解释规范化、Dirty/Touched、字段修订、三方比较、列表投影、乐观更新、自动保存与导航恢复；完整桌面观察页演示保存期间继续输入、版本冲突、拒绝、撤销及切换对象后的迟到响应，内存模型不冒充真实数据库。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：BIZ-06](../chinese-guides/biz-06-async-jobs-import-export-progress.md#biz-06)、[MDN AbortController](https://developer.mozilla.org/zh-CN/docs/Web/API/AbortController)。覆盖范围：独立主讲义沿长任务资源串联意图、任务和执行尝试，区分任务状态与页面连接、阶段进度与完成、快照与增量推送，补充有限退避轮询、取消检查点、租约与 fencing token、部分导入报告、结果领取及三种期限；MDN 只用于查证客户端取消信号，Abort 不代表后台任务已经停止或回滚。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：BIZ-07](../chinese-guides/biz-07-errors-idempotency-eventual-consistency.md#biz-07)、[MDN 条件请求：乐观锁与 412](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Guides/Conditional_requests#使用乐观锁避免更新丢失问题)。覆盖范围：独立主讲义从已知失败与未知结果推进到意图幂等、同键异载荷、版本前提、原子提交、Outbox、消息去重和增量缺口；以完整可运行模型说明重复效果与乱序处理，继续覆盖最终一致、独立补偿意图、有限重试、对账与故障位置证据。MDN 用于查证 ETag/If-Match，不将内存演示当作分布式事务保证。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：UX-01](../chinese-guides/ux-01-interaction-states-usability-validation.md#ux-01)。覆盖范围：独立主讲义以资料修正连接任务、信息架构、状态、反馈、部分成功恢复、焦点、渐进披露、原型、研究、指标和交付；提供结果动作、重提范围、合成观察统计及可操作页面，区分发送前离线与结果未知，统计不冒充真实用户研究；布局验证聚焦桌面窗口、放大与长文本。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义：BIZ-08](../chinese-guides/biz-08-requirement-acceptance-traceability.md#biz-08)。覆盖范围：独立主讲义以代班复核串联目标、规则、验收与执行，补充到期边界、影响关系遍历和证据基线的可运行例子；覆盖稳定 ID、版本、双向追踪、矩阵、ADR、非功能条件、AI 辅助与生产差异，明确链接存在不证明真实执行。AI Agent 执行合同另归 AIDEV-01。
