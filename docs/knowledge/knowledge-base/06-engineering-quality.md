...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义](../chinese-guides/git-01-object-index-references-recovery.md#git-01)、[中文｜Pro Git 记录每次更新](https://git-scm.com/book/zh/v2/Git-基础-记录每次更新到仓库)、[中文｜Pro Git Git 对象](https://git-scm.com/book/zh/v2/Git-内部原理-Git-对象)、[中文｜Pro Git 重置揭密](https://git-scm.com/book/zh/v2/Git-工具-重置揭密)。覆盖范围：以同一文件的三个版本串起工作区、暂存区、diff 与提交，解释对象、引用、restore/reset/revert、reflog 救援、detached HEAD、未跟踪与备份边界；配连续 PowerShell 练习和结果说明，无硬前置，关联分支、协作与版本调试。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义](../chinese-guides/git-02-branches-merge-rebase-conflicts.md#git-02)、[中文｜Pro Git 分支简介](https://git-scm.com/book/zh/v2/Git-分支-分支简介)、[中文｜Pro Git 分支的新建与合并](https://git-scm.com/book/zh/v2/Git-分支-分支的新建与合并)、[中文｜Pro Git 变基](https://git-scm.com/book/zh/v2/Git-分支-变基)。覆盖范围：用同一分叉对照 merge、rebase、cherry-pick 与快进，逐步读取共同祖先、点号比较、三阶段索引及 ours/theirs，保留冲突双方需求并核对组合行为；补充历史整理、abort、撤销 merge 与集成边界，直接前置仅 GIT-01。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义](../chinese-guides/git-03-commits-remotes-pr-worktrees-collaboration.md#git-03)、[中文｜Pro Git 向项目贡献](https://git-scm.com/book/zh/v2/分布式-Git-向一个项目贡献)、[中文｜GitHub 关于拉取请求](https://docs.github.com/zh/pull-requests/reference/pull-requests)、[中文｜GitHub 审查建议更改](https://docs.github.com/zh/pull-requests/how-tos/review-pull-requests/reviewing-proposed-changes-in-a-pull-request?tool=webui)。覆盖范围：以原子提交和可解释的说明起步，用本地裸远端、双副本与 worktree 演示同步、分叉、显式 lease 拒绝和现场隔离，连接 PR、合入方式、运行资源、submodule/LFS 与交付证据；直接前置仅 GIT-02。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-01-module-graph-build-output-source-maps.md#eng-01)。覆盖范围：用完整资料应用沿模块图观察转换、打包、压缩、manifest、动态与静态导入对照、CSS 加载、tree-shaking 与必要副作用；用实际生成堆栈和同次 Source Map 还原源码，连接库产物、执行位置、体积和可复现输入；直接前置仅 JS-06。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-02-dev-production-environments-assets-cache.md#eng-02)。覆盖范围：延续构建应用，对照开发/HMR、生产构建、预览、mode 与环境值解析；用子路径和本地 HTTP 服务观察 HTML 回退、真实 404、ETag/304 与缓存响应头，再解释旧页面、灰度兼容、运行配置和 Service Worker；直接前置仅 ENG-01。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-03-dependencies-lockfile-workspaces-peer.md#eng-03)、[中文｜pnpm Workspaces（官方当前版）](https://pnpm.io/zh/workspaces)、[中文｜语义化版本 2.0.0](https://semver.org/lang/zh-CN/)（只用于核验公共 API 与 MAJOR/MINOR/PATCH 版本承诺，不要求记忆页面后半的 BNF 语法和正则）。覆盖范围：用纯本地 workspace 与独立 tarball 消费者解释依赖类别、清单/锁文件/解析图、workspace 缺包拒绝、冻结安装、离线内容、SemVer、exports、重复实例和 peer；保留多包任务、override、补丁、catalog、来源/安装脚本/许可及制品边界，无硬前置。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-05-quality-gates-lint-types-tests-ci.md#eng-05)。覆盖范围：以一个可独立安装的统计包逐项对照 format、lint、类型、行为和构建，用故意失败与转译例子解释证明边界及退出状态传播；连接 CI 输入、受影响范围、缓存失效、实际合入提交、必需检查、例外与可行动诊断；直接前置仅 ENG-03。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/test-01-test-design-oracles-properties-mutation.md#test-01)、[Vitest Writing Tests](https://cn.vitest.dev/guide/learn/writing-tests)。覆盖范围：以资料标题的中文与 emoji 长度规则给出可运行的边界用例、fast-check 属性与四种手工变异；通过受控时间和 Promise 解释预言、状态、不变量、替身、覆盖率与失败判断，并比较蜕变、差分、fuzz 和契约的边界；无硬前置。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文工程讲义：TEST-02](../chinese-guides/test-02-component-testing-user-behavior-accessibility.md#test-02)、[Testing Library Queries](https://testing-library.node.org.cn/docs/queries/about/)、[User Event](https://testing-library.node.org.cn/docs/user-event/intro/)。覆盖范围：给出独立 React 表单项目，以键盘校验、保存中反馈、受控失败、草稿保留、真实操作语义与只读三个用例解释角色查询、异步等待和公开行为；区分请求替身、权限、框架资源、SSR 与浏览器布局；直接前置为 TEST-01、REACT-02。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文工程讲义：TEST-03](../chinese-guides/test-03-e2e-visual-regression-isolation-flakiness.md#test-03)、[Playwright Introduction](https://playwright.nodejs.cn/docs/intro)、[Best Practices](https://playwright.nodejs.cn/docs/best-practices)、[截图视觉比较](https://playwright.nodejs.cn/docs/test-snapshots)。覆盖范围：给同一表单接上 HTTP 服务，以保存后刷新、一次故障后的真实重试和桌面截图展示关键任务；讲清语义等待、浏览器与服务数据隔离、视觉审查、Trace、重试和 flaky 归因；明确内存存储、本地环境与生产证明边界；直接前置为 TEST-01、WEB-01。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[TEST-04 契约兼容完整讲义](../chinese-guides/test-04-api-contract-consumer-driven-testing.md#test-04)。覆盖范围：独立主讲义以阅读页、编辑页和索引任务说明真实消费代码、必要匹配、Provider State、契约制品和提供者验证，提供消费失败、字段反例与三态版本矩阵；覆盖 HTTP/Protobuf/消息兼容、发布窗口、诊断与检查投入，明确自编模型未实现 Pact/Broker，也不替代真实代理与业务验证。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文｜ENG-08 供应链完整讲义](../chinese-guides/eng-08-software-supply-chain-sbom-provenance.md#eng-08)。覆盖范围：独立主讲义以同一制品摘要连接依赖来源、SBOM 范围、漏洞与许可、provenance、签名身份、构建隔离、消费政策、晋级与响应；提供证据关联、三态结论、消费者查询以及真实 SHA-256/Ed25519 实验，明确清单与签名不能代替安全或许可结论，本地实验不实现 Sigstore/SLSA 平台保证。
