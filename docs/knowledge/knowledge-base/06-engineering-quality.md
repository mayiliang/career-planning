...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[TEST-04 契约兼容完整讲义](../chinese-guides/test-04-api-contract-consumer-driven-testing.md#test-04)。覆盖范围：独立讲义从接口契约风险、消费者期望与 Provider State 推进到 Broker、提供者验证、多消费者/多版本矩阵、HTTP/Protobuf/消息协议、兼容演进、发布阻断和故障诊断；本点负责可执行验证，不重复 BIZ-04 的业务 DTO 设计或 API-01/02 的规范解析器。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文｜ENG-08 供应链完整讲义](../chinese-guides/eng-08-software-supply-chain-sbom-provenance.md#eng-08)。覆盖范围：独立讲义覆盖依赖/lockfile、SBOM、许可证、漏洞、构建材料、provenance、attestation、SLSA、制品晋级、消费端验证和事件响应，并明确各类证据不能互相替代；AI 建议依赖专项审查归 `AIDEV-07`，容器层硬化归 `DOCKER-04`。
