...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义](../chinese-guides/git-01-object-index-references-recovery.md#git-01)、[中文｜Pro Git 记录每次更新](https://git-scm.com/book/zh/v2/Git-基础-记录每次更新到仓库)、[中文｜Pro Git Git 对象](https://git-scm.com/book/zh/v2/Git-内部原理-Git-对象)、[中文｜Pro Git 重置揭密](https://git-scm.com/book/zh/v2/Git-工具-重置揭密)。覆盖范围：独立主讲义从内容寻址对象和三层状态开始，连续讲解 blob/tree/commit、引用与可达性、`restore`/`reset`/`revert` 的作用层、reflog 恢复、敏感信息边界、部分暂存、裸仓库与可复核恢复证据；本讲无硬前置，Pro Git 只作中文官方机制补充。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文核心讲义](../chinese-guides/git-02-branches-merge-rebase-conflicts.md#git-02)、[中文｜Pro Git 分支简介](https://git-scm.com/book/zh/v2/Git-分支-分支简介)、[中文｜Pro Git 分支的新建与合并](https://git-scm.com/book/zh/v2/Git-分支-分支的新建与合并)、[中文｜Pro Git 变基](https://git-scm.com/book/zh/v2/Git-分支-变基)。覆盖范围：独立主讲义以提交图和共同祖先为主线，串起 fast-forward、三方合并、rebase、cherry-pick、三阶段索引、文本/语义冲突、共享历史、撤销 merge 与集成队列；头部只直接链接 GIT-01，Pro Git 用于补充官方命令机制。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-01-module-graph-build-output-source-maps.md#eng-01)。覆盖范围：独立主讲义从入口建立模块图，连续解释转换、打包、chunk、tree-shaking、副作用、CSS/资源、manifest、Source Map、共享缓存、多环境运行时、体积预算与可复现构建；头部只直接链接 JS-06，不再要求先读合并工程提纲。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-02-dev-production-environments-assets-cache.md#eng-02)。覆盖范围：独立主讲义逐层区分开发服务器/HMR、生产构建、mode 与环境变量、代理、base、SPA fallback、内容散列、Service Worker、新旧版本兼容、运行时配置、灰度和真实入口验证；头部只直接链接 ENG-01。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-03-dependencies-lockfile-workspaces-peer.md#eng-03)、[中文｜pnpm Workspaces（官方当前版）](https://pnpm.io/zh/workspaces)、[中文｜语义化版本 2.0.0](https://semver.org/lang/zh-CN/)（只用于核验公共 API 与 MAJOR/MINOR/PATCH 版本承诺，不要求记忆页面后半的 BNF 语法和正则）。覆盖范围：独立主讲义从 manifest 意图、解析图、lockfile 与物理布局推进到 peer、workspace、override、条件导出、供应链、安装脚本、许可和干净环境验证；本讲无硬前置，两个官方中文页只作规则补充。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/eng-05-quality-gates-lint-types-tests-ci.md#eng-05)。覆盖范围：独立主讲义按风险区分 format、lint、typecheck、测试和构建，解释本地反馈、CI 裁决、可信缓存、历史基线、例外生命周期、required check、紧急通道、供应链权限与质量趋势；头部只直接链接 ENG-03。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文完整讲义](../chinese-guides/test-01-test-design-oracles-properties-mutation.md#test-01)、[Vitest Writing Tests](https://cn.vitest.dev/guide/learn/writing-tests)。覆盖范围：独立主讲义从预言、等价类、边界与状态转换推进到替身、依赖注入、属性/蜕变/差分/fuzz/变异测试、覆盖率边界、非确定性、契约、统计预言和失败分类；本讲无硬前置，Vitest 页只补充框架书写接口。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文工程讲义：TEST-02](../chinese-guides/test-02-component-testing-user-behavior-accessibility.md#test-02)、[Testing Library Queries](https://testing-library.node.org.cn/docs/queries/about/)、[User Event](https://testing-library.node.org.cn/docs/user-event/intro/)。覆盖范围：独立主讲义从组件测试边界、角色/名称查询与真实事件推进到异步、网络、时间、Portals、可访问性、SSR/hydration 与跨框架行为合同；头部直接链接 TEST-01 与 REACT-02，二者的更早基础继续递归展开。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文工程讲义：TEST-03](../chinese-guides/test-03-e2e-visual-regression-isolation-flakiness.md#test-03)、[Playwright Introduction](https://playwright.nodejs.cn/docs/intro)、[Best Practices](https://playwright.nodejs.cn/docs/best-practices)、[截图视觉比较](https://playwright.nodejs.cn/docs/test-snapshots)。覆盖范围：独立主讲义围绕跨边界用户任务讲清 locator、自动等待、数据/身份/worker 隔离、trace、并行、重试与 flaky 归因，再独立处理视觉基线、字体/动画/阈值和更新审查；头部直接链接 TEST-01 与 WEB-01。
