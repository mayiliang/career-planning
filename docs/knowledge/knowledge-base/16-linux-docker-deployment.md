...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文学习讲义：LINUX-01](../chinese-guides/linux-01-filesystem-permissions-safe-commands.md#linux-01)。覆盖范围：独立主讲义从路径、挂载、目录项与 inode 解释目录权限、身份组选择、umask、链接、诊断、特殊参数、原子替换、归档、ACL 与安全修改；提供三段内存模型与完整临时目录 Bash 实验，明确基本权限不涵盖全部内核策略、原子不等于持久、检查路径不消除竞争，Linux 实机证据与 Windows 语法检查分开。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文学习讲义：LINUX-02](../chinese-guides/linux-02-process-port-log-network-diagnostics.md#linux-02)。覆盖范围：独立主讲义沿浏览器、代理和上游解释进程/systemd、socket、回环、DNS、双栈、路由、TLS、curl 阶段时间、日志、资源与信号；提供命令、示意输出和完整本地 HTTP 对照实验，区分 HTTP 状态与传输退出码、137 与 OOM、原生程序验证与 Linux 实机证据。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文学习讲义：LINUX-03](../chinese-guides/linux-03-shell-environment-automation.md#linux-03)。覆盖范围：独立主讲义从参数与失败解释引用、数组、管道、errexit 例外、环境、秘密、dry-run、锁、暂存、原子提交、幂等、trap、网络预算及定时任务；提供三份 Shell 语义实验和完整临时目录发布脚本，明确预览不预订状态、锁不代替安全边界、清理不自动回滚、Persistent 不逐条重放。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文学习讲义：LINUX-04](../chinese-guides/linux-04-server-security-ssh-users-firewall.md#linux-04)。覆盖范围：独立主讲义从允许矩阵解释间接权限、SSH 服务器与用户身份、Include/Match 有效配置、key 轮换撤销、sudo 执行链、双栈与 Docker 防火墙、服务硬化、日志备份和事件恢复；补充配置片段与允许/拒绝对照，明确 key 注释不自动到期、旧会话不证明新登录、部署代码能力与运行身份相连。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文学习讲义：DOCKER-01](../chinese-guides/docker-01-images-containers-dockerfile-cache.md#docker-01)。覆盖范围：独立主讲义围绕镜像与容器、tag/digest、构建上下文、缓存、多阶段、基础平台、秘密、权限、信号、端口和健康；提供完整无第三方依赖的资料页项目，区分应用原生运行与容器实证、缓存与可重现、unhealthy 与自动重启，说明教学 tag 的可变性。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[中文学习讲义：DOCKER-02](../chinese-guides/docker-02-compose-network-volumes-environments.md#docker-02)。覆盖范围：独立主讲义连接项目归属、服务名、观察位置、端口发布、插值/环境、启动健康、迁移、卷、bind mount、合并覆盖、资源与恢复；提供完整网关/单写入者计数项目，明确卷不是备份、健康不等于业务就绪、文件实验不提供数据库事务与多副本保证、回滚需要数据兼容。
...
- [ ] 自评已掌握
- [ ] 已通过严格考核
- 学习资料：[DEPLOY-01 中文核心讲义](../chinese-guides/deploy-01-nginx-static-assets-reverse-proxy-https-cdn.md#deploy-01)。该讲义从请求路径逐层讲解静态资源、反向代理、TLS 和 CDN 缓存的联动边界；覆盖范围：独立主讲义围绕请求分类解释 location、root/alias、try_files、proxy_pass URI、代理超时/缓冲、TLS、缓存指令、共享键、压缩、清理与资源兼容窗口；提供完整本地 Nginx 配置、Node JSON 上游和响应矩阵，区分原生示例验证与 Nginx/TLS/CDN 实机证据。
