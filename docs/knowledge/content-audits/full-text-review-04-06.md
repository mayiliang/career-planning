...
2026-09-23：B25 的 ENG-06、OBS-01 已按知识点重写，补充中文情境、可观察例子、概念交叉引用与桌面核对；本地实验与尚未执行的 Nginx/TLS/CDN、真实发布和真实访问测量分别记录，见[学习资料系统的 B25 记录](../../development/11-learning-material-system.md#b25-的版本交付与用户体验)。后文保留历史审校记录。

2026-09-22：B23 的 UX-01、ENG-08 已按知识点重写，补充示例、交叉引用与桌面阅读核对；内容范围、运行证据及 Linux 实验尚未实机执行的限制见[学习资料系统的 B23 记录](../../development/11-learning-material-system.md#b23-的交互所有权与交付证据)。后文保留历史审校记录。

2026-09-20：B21 的 BIZ-08、TEST-04 已重写为知识点讲义，补充可运行例子、跨篇关联及桌面阅读核对；当前内容和验证边界见[学习资料系统的 B21 记录](../../development/11-learning-material-system.md#b21-的交付追踪与渲染边界)。后文保留历史审校记录。

2026-09-20：B20 的 BIZ-03 至 BIZ-07 已重构为同一资料管理场景下的五篇独立讲义，补充可运行例子、编辑观察页、跨文档关联与官方语义核对；当前内容和验证边界见[学习资料系统的 B20 记录](../../development/11-learning-material-system.md#b20-的权限合同与一致性恢复)。后文保留历史审校记录。

2026-09-19：B19 的 BIZ-01、BIZ-02 已按知识点重新组织中文解释与自编场景，补充可运行例子、概念关联和桌面阅读核对；当前内容与验证边界见[学习资料系统的 B19 记录](../../development/11-learning-material-system.md#b19-的上下文证据与业务模型)。后文保留原日期的历史审校记录。
...
| 2026-08-31 独立中文主讲义 | 全文从业务问题自然推进到实体、值对象、聚合、不变量、统一语言、限界上下文、关系、DTO/领域/展示模型、事件、并发、前端映射与演进；例子用于解释而非组织全部正文。 | 不教授 DDD 战略设计全套，也不绑定微服务或 ORM。 | 核心必读。 |

闭环：主讲义可独立建立对象、语言、一致性和前端映射模型；Microsoft 中文页只核对 DDD 核心概念。固定报名输入仍可用于独立考核，但不再反向决定正文结构。
...
| 2026-08-31 独立中文主讲义 | 全文解释适用性、命令/事件/状态、转移表、不变量、纯守卫、终态、幂等、版本并发、领域时间、outbox/补偿、层级/并行、演进、UI 投影、可观测与状态空间。 | 不要求 SCXML/XState 实现；固定 o7/k9 只作为一种演练。 | 唯一中文首学主源。 |

2026-08-31 的 B19 复核逐份读完 BIZ-01、BIZ-02 现行主讲义及递归前置。BIZ-01 无硬前置；BIZ-02 只直接列 BIZ-01 与 TS-02，后者继续递归列自己的 TypeScript/JavaScript 前置，因此父讲义不重复祖先。两条直接依赖互不包含，无环且无传递性重复。Microsoft 中文 DDD 页和 XState 英文概念页分别只承担概念核对与版本术语核验。
