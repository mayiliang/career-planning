...
15. 正常启动必须自动迁移、增量同步知识、在空库生成 23 周计划并创建当天备份；不得要求用户先进入初始化页面。
...
18. 学习计划按 15 分钟粒度拆分阶段，前 18 个阶段覆盖 153 个唯一知识点；服务端和客户端按北京时间解释日期，计划从当前北京时间日期开始排布，默认每日容量 540 分钟，每天预计投入 540 分钟，周一至周日连续推进且没有休息日概念。
...
8. 知识模块默认入口是体系脑图：先展示 6 条能力主干和 16 个领域，再按需展开知识点；清单和关系图谱作为等价视图保留。
...
4. 日历、打卡、请假顺延、周复盘和每周 7 天的 23 周计划可用。
