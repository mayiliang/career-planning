...
15. 正常启动必须自动迁移、增量同步知识、在空库生成 16 周计划并创建当天备份；不得要求用户先进入初始化页面。
...
18. 学习计划按 15 分钟粒度拆分阶段，前 15 周覆盖 143 个唯一知识点，单日不超过 480 分钟，周日固定 210 分钟综合闸门，第 16 周不新增知识点。
...
8. 知识模块默认入口是体系脑图：先展示 5 条能力主干和 15 个领域，再按需展开知识点；清单和关系图谱作为等价视图保留。
...
4. 日历、打卡、请假顺延、周复盘和每周 7 天的 16 周计划可用。
