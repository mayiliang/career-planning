...
| `reset` | 重试原因 | 最终正文不完整时清空未完成残稿，准备自动重试 |
| `done` | 最终 DTO | 服务端校验、保存完成 |
| `error` | 中文 `message` | 可显示错误 |

服务端识别兼容接口常见的 `reasoning_content` 和 `reasoning` 字段；thinking 与最终正文分别累加、分别渲染。thinking 只作为模型生成过程的可见信息，不作为事实依据、评分证据或持久化的用户笔记。笔记整理若已经产生 thinking、但最终 JSON 正文被截断或缺失，系统会清空正文残稿并自动进行一次“只生成最终正文”的重试；重试仍失败时才生成明确标注的本地安全排版稿，不能把 thinking 冒充整理正文。
...
- thinking 使用有高度上限的独立滚动区：默认跟随最新增量；用户向上滚动后暂停跟随并提供“回到最新”；流结束后自动收起。
...
3. 模型输出 thinking 时，折叠区在正文前流式更新、有明确展开状态且不超过视口约三分之一；不输出时不显示空面板。
